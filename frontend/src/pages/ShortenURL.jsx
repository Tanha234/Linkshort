...
const ShortenURL = ({ backendUrl }) => {
  ...
  const handleShorten = async () => {
    ...
    const userId = "demoUserId"; // Replace with actual user ID if you have authentication
  
    try {
      const res = await fetch(`${backendUrl}/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ originalUrl: longUrl, userId, shortCode, shortUrl }),
      });
  
      const data = await res.json();
      if (res.ok) {
        setShortUrl(data.shortUrl); // Use returned shortUrl
